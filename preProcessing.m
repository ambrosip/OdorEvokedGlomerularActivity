...
assumptions and alerts:
    ...
    !!search for "assumptions" and "alerts" to find up-to-date info in code
...
% % get info from 1st "*Events.csv" found
% olfactometer_file_name = dir(fullfile(expDir, '**','*Events.csv')).name;
% olfactometer_file_folder = dir(fullfile(expDir, '**','*Events.csv')).folder;
% olfactometer_file_dir = fullfile(olfactometer_file_folder, olfactometer_file_name);
    ...
    if s_olfactometer.(programFieldName).type ~= "ignore"
        ...
        if olfactory_task == "2afc_fine_coarse_fine"
            for trialNum = 1:trialNum_total
                startMin_this_trial = s_olfactometer.(programFieldName).startMin_by_trial(trialNum);
                first_event_idx_this_trial = find(x_minutes == startMin_this_trial, 1, 'first');
                % check if this is the last trial
                if trialNum == trialNum_total
                    % it this is the last trial, set startMin_next_trial to end of x_minutes
                    startMin_next_trial = max(x_minutes);
                else
                    % it this is NOT the last trial, set startMin_next_trial appropriatedly
                    startMin_next_trial = s_olfactometer.(programFieldName).startMin_by_trial(trialNum + 1);
                end
                last_event_idx_this_trial = find(x_minutes == startMin_next_trial, 1, 'last');
                search_subset = s_olfactometer.(programFieldName).file.Events(first_event_idx_this_trial:last_event_idx_this_trial);
                response_window_start_idx = find(strcmp(search_subset,'Response'));
                search_subset = s_olfactometer.(programFieldName).file.Events(response_window_start_idx:last_event_idx_this_trial);
                % label outcomes
                if ~isempty(find(contains(search_subset,'Reward'),1))
                    s_olfactometer.(programFieldName).outcome_by_trial(trialNum,1) = "hit";
                elseif ~isempty(find(contains(search_subset,'Lick'),1))
...
% main output: struct_acqNum_by_odor and labeled_acqs
