...
% Create a UI Figure
fig = uifigure();
fig.Position = [800 150 700 800];

% Create UserData struct with default values
fig.UserData.original = zScoreEnvelope;
fig.UserData.image = zScoreEnvelope;
fig.UserData.blurStd = 1.0;
fig.UserData.threshold = 1.0;
fig.UserData.mask = zeros(size(zScoreEnvelope));
fig.UserData.maskAfterExclusion = zeros(size(zScoreEnvelope));
fig.UserData.plotRange = [-5 5];
fig.UserData.gradient = divergingGradient;
fig.UserData.showThreshold = true;
fig.UserData.showROIs = true;
fig.UserData.currentTool = "Pick Strip Center";
fig.UserData.excludedCenter = round(size(zScoreEnvelope, 1) / 2);
fig.UserData.excludedHeight = 50;
...
showThreshold.ValueChangedFcn = ...
    @(src, event) thresholdButton(src, fig);

showROIs = uibutton(glButtons, "state");
showROIs.Text = "Hide ROIs";
showROIs.ValueChangedFcn = ...
    @(src, event) ROIButton(src, fig);

tools = uibuttongroup(glButtons);
tools.SelectionChangedFcn = @(src, event) toolsCallback(src, fig);
...
fig.UserData.axis = uiaxes(gl);
fig.UserData.axis.Layout.Row = 2;
fig.UserData.axis.Layout.Column = 1;

fig.UserData.plotHandle = imshow(fig.UserData.image ...
                                , fig.UserData.plotRange ...
                                , 'Parent' ...
                                , fig.UserData.axis);
colormap(fig.UserData.axis, fig.UserData.gradient);

% Add excluded region to the plot
imgWidth = size(fig.UserData.image, 2);
excludedPosition = [ 1 ...
                   ( fig.UserData.excludedCenter ...
                       - round(fig.UserData.excludedHeight / 2)) ...
                   imgWidth ...
                   fig.UserData.excludedHeight ...
                   ];

fig.UserData.excluded = rectangle( "Parent", fig.UserData.axis ...
                                 , "FaceColor", [.8 .8 0.0] ...
                                 , "FaceAlpha", 0.5 ...
...
thresholdSlider.Limits = [0 5];
thresholdSlider.Value = fig.UserData.threshold;
thresholdSlider.MajorTicks = 0:5;
thresholdSlider.MinorTicks = 0:.25:5;

thresholdSlider.ValueChangedFcn = ...
    @(src, event) updateThreshold(event, fig);
...
blurSlider.Limits = [0 5];
blurSlider.Value = fig.UserData.blurStd;
blurSlider.MajorTicks = 0:5;
blurSlider.MinorTicks = 0:.25:5;

blurSlider.ValueChangedFcn = ...
    @(src, event) updateBlur(event, fig);
...
excludedSlider.Value = fig.UserData.excludedHeight;
excludedSlider.MajorTicks = 0:100:400;
excludedSlider.MinorTicks = 0:50:400;

excludedSlider.ValueChangedFcn = ...
    @(src, event) updateExcludedRegion(event.Value, fig);

% Add click callback
fig.WindowButtonDownFcn = @(src, event) clickCallback(fig);

% Add mask to plot
updateExcludedRegion(fig.UserData.excludedHeight, fig);

fig.UserData.overlayHandle = ...
    imshow(fig.UserData.maskAfterExclusion, fig.UserData.plotRange, ...
        'Parent', fig.UserData.axis);
colormap(fig.UserData.axis, fig.UserData.gradient);

redraw(fig);

hold(fig.UserData.axis, 'off');
