...
% Baseline right before analysis window (baseline_pre_odor = false) 
% or right before odor presentation (baseline_pre_odor = true)
baseline_pre_odor = true;

% Number of subdivisions of the post odor interval
nSubdivisions = 5;
...
%   2 - Signal interval is the post-odor presentation interval
%   3 - Baseline interval is the pre-odor presentation interval

frameOdorOnset = round(odor_onset_s * frame_rate_hz);
frameOnset = round((odor_onset_s + post_odor_interval) * frame_rate_hz);
frameOffset = round((odor_offset_s + post_odor_interval) * frame_rate_hz);

% Number of frames between onset and offset (including both ends)
frameDuration = frameOffset - frameOnset + 1;

if baseline_pre_odor
    % Baseline starts one window duration before the odor onset
    frameBaselineStart = frameOdorOnset - frameDuration;
    figNameSuffix = strcat('_', num2str(post_odor_interval), 's_afterOdor_vs_preOdor');
else
    % Baseline starts one window duration before the analysis onset
    frameBaselineStart = frameOnset - frameDuration;
    figNameSuffix = strcat('_', num2str(post_odor_interval), 's_afterOdor_vs_odor');
end
    ...
    for odor = programOdors'
        ...
        for row = 1:height(odorTable)
            ...
            frames = single(read_file( ...
                filepath, frameBaselineStart, frameDuration)) + 32768;
            baselineImage = mean(frames, ndims(frames));

            % Load frames for the signal window
            frames = single(read_file( ...
                filepath, frameOnset, frameDuration)) + 32768;
...
if plotTogether
    ...
    fig = figure('Name', strcat(figName, figNameSuffix));
    ...
    title(tl, strcat(figName, figNameSuffix), 'Interpreter', 'none', 'FontSize', 16);
    ...
    for iFigure = 1:length(figures)
        ...
        title(tl, strcat(figName, figNameSuffix), 'Interpreter', 'none');
...
matFileName = strcat(imgsToAnalyzeNames{1}(1:end-9),'_',imgsToAnalyzeNames{end}(end-13:end-4),'_postOdor');
