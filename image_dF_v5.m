...
for program_name = string(fieldnames(s_olfactometer))'
    ...
    for odor = programOdors'
        ...
        % Initialize images
        figures(iFigure).hit.image_dFF = zeros(frameSize);
        figures(iFigure).miss.image_dFF = zeros(frameSize);
        figures(iFigure).false.image_dFF = zeros(frameSize);
        figures(iFigure).na.image_dFF = zeros(frameSize);
        figures(iFigure).hit.image_zScore_v1 = zeros(frameSize);
        figures(iFigure).miss.image_zScore_v1 = zeros(frameSize);
        figures(iFigure).false.image_zScore_v1 = zeros(frameSize);
        figures(iFigure).na.image_zScore_v1 = zeros(frameSize);
        figures(iFigure).hit.image_zScore_v2 = zeros(frameSize);
        figures(iFigure).miss.image_zScore_v2 = zeros(frameSize);
        figures(iFigure).false.image_zScore_v2 = zeros(frameSize);
        figures(iFigure).na.image_zScore_v2 = zeros(frameSize);
