% Made to deal with experiments in which saving of data got interrupted 
% by lack of space, such as M:\ImagingData\20260316\m357\e1
...
acq_idx = 1;
trial_locs_idx = 1;
trial_locs_idx_max = size(trial_locs,1);
acq_idx_max = size(acq_list,1);
...
if size(trial_locs,1) ~= size(rawImgStarts_min,1)  
    ...
    for programNum = 1:size(programFieldNames,1)         
        ...
        if s_olfactometer.(programFieldName).type ~= "ignore" 
            ...
            for trialNum = 1:trialNum_total
                if trial_locs_idx <= trial_locs_idx_max && acq_idx <= acq_idx_max
    ...
    for programNum = 1:size(programFieldNames,1)      
        ...
        if s_olfactometer.(programFieldName).type ~= "ignore" 
            ...
            % iterate from 1st to last trial
            for trialNum = 1:trialNum_total
